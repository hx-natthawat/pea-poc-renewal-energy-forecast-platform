...
INSERT INTO network_transformers (id, name, name_th, rated_power_kva, primary_voltage_kv, secondary_voltage_v, max_current_a)
VALUES ('TX_50KVA_01', 'POC Distribution Transformer', 'หม้อแปลงทดสอบสาธิต', 50, 22, 400, 72.2)
...
INSERT INTO network_nodes (id, name, node_type, phase, nominal_voltage_v, transformer_id, distance_from_tx_m) VALUES
    ('node_lv_bus', 'LV Busbar', 'slack', 'N', 400, 'TX_50KVA_01', 0),
    -- Phase A nodes (prosumer3 -> prosumer2 -> prosumer1)
    ('node_a1', 'Phase A Node 1', 'pq', 'A', 230, 'TX_50KVA_01', 50),
    ('node_a2', 'Phase A Node 2', 'pq', 'A', 230, 'TX_50KVA_01', 100),
    ('node_a3', 'Phase A Node 3', 'pq', 'A', 230, 'TX_50KVA_01', 150),
    -- Phase B nodes (prosumer6 -> prosumer4 -> prosumer5)
    ('node_b1', 'Phase B Node 1', 'pq', 'B', 230, 'TX_50KVA_01', 50),
    ('node_b2', 'Phase B Node 2', 'pq', 'B', 230, 'TX_50KVA_01', 100),
    ('node_b3', 'Phase B Node 3', 'pq', 'B', 230, 'TX_50KVA_01', 150),
    -- Phase C nodes (prosumer7 only)
    ('node_c1', 'Phase C Node 1', 'pq', 'C', 230, 'TX_50KVA_01', 50)
ON CONFLICT (id) DO NOTHING;

-- Link prosumers to nodes
UPDATE network_nodes SET prosumer_id = 'prosumer3' WHERE id = 'node_a1';
UPDATE network_nodes SET prosumer_id = 'prosumer2' WHERE id = 'node_a2';
UPDATE network_nodes SET prosumer_id = 'prosumer1' WHERE id = 'node_a3';
UPDATE network_nodes SET prosumer_id = 'prosumer6' WHERE id = 'node_b1';
UPDATE network_nodes SET prosumer_id = 'prosumer4' WHERE id = 'node_b2';
UPDATE network_nodes SET prosumer_id = 'prosumer5' WHERE id = 'node_b3';
UPDATE network_nodes SET prosumer_id = 'prosumer7' WHERE id = 'node_c1';
...
INSERT INTO network_branches (id, name, branch_type, from_node_id, to_node_id, length_m, r_ohm_per_km, x_ohm_per_km, max_current_a) VALUES
    -- Phase A feeder
    ('branch_a0', 'LV Bus to A1', 'cable', 'node_lv_bus', 'node_a1', 50, 0.32, 0.08, 200),
    ('branch_a1', 'A1 to A2', 'cable', 'node_a1', 'node_a2', 50, 0.32, 0.08, 200),
    ('branch_a2', 'A2 to A3', 'cable', 'node_a2', 'node_a3', 50, 0.32, 0.08, 200),
    -- Phase B feeder
    ('branch_b0', 'LV Bus to B1', 'cable', 'node_lv_bus', 'node_b1', 50, 0.32, 0.08, 200),
    ('branch_b1', 'B1 to B2', 'cable', 'node_b1', 'node_b2', 50, 0.32, 0.08, 200),
    ('branch_b2', 'B2 to B3', 'cable', 'node_b2', 'node_b3', 50, 0.32, 0.08, 200),
    -- Phase C feeder
    ('branch_c0', 'LV Bus to C1', 'cable', 'node_lv_bus', 'node_c1', 50, 0.32, 0.08, 200)
ON CONFLICT (id) DO NOTHING;
...
SELECT create_hypertable('doe_limits', 'time',
...
INSERT INTO network_constraints (id, constraint_type, limit_value, unit, applies_to, priority) VALUES
    ('voltage_upper', 'voltage_upper', 242, 'V', 'all', 1),  -- +5%
    ('voltage_lower', 'voltage_lower', 218, 'V', 'all', 1),  -- -5%
    ('thermal_cable', 'thermal', 200, 'A', 'all', 2),
    ('thermal_tx', 'thermal', 72.2, 'A', 'TX_50KVA_01', 1)
ON CONFLICT (id) DO NOTHING;
...
WHERE n.id != 'node_lv_bus'
ORDER BY n.phase, n.distance_from_tx_m;

-- =============================================================================
-- COMMENT
-- =============================================================================
COMMENT ON TABLE network_transformers IS 'Distribution transformers for power flow analysis';
COMMENT ON TABLE network_nodes IS 'Network buses/nodes for topology modeling';
COMMENT ON TABLE network_branches IS 'Lines and cables connecting nodes';
COMMENT ON TABLE doe_limits IS 'Calculated Dynamic Operating Envelope limits per prosumer';
COMMENT ON TABLE network_constraints IS 'Operating constraints for DOE calculations';
